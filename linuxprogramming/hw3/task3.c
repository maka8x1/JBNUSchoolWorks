...
void clink(char *l_type, char *source, char *dest) { 
	...
	if(strcmp(l_type, "-h") == 0){ 
		if(link(source, dest) == -1) { 
			printf("Error: failed to make hard link\n");
		} 
	} else if(strcmp(l_type, "-s") == 0) { 
		if(symlink(source, dest) == -1) { 
			printf("Error: failed to make soft link\n");
		}
	...
	while(1) {
		...
		if(strstr(raw_command, "ls") != NULL) {
			...
			if(args != NULL) { 
                ...
                } else if(strstr(raw_command, "link") != NULL) {
                        char* args = strtok(raw_command, " ");
                        args = strtok(NULL, " ");
