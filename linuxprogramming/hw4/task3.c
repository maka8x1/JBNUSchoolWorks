...
// Excute Process, BG
void excutebg(char** inputs, int lengths) {
        int pid, stat;

	char** new_inputs = (char **)malloc(sizeof(char*) * (lengths));
        for(int i = 0; i < lengths - 1; i++) {
                new_inputs[i] = inputs[i];
        }
        new_inputs[lengths - 1] = NULL;
...
void prompt() { 
	char raw[2048];
	char** args = (char **)malloc(sizeof(char*) * 128); // maximum arguments = 128
	...
	if (count > 0) { 
		...
		if(evaluatecmd(args[0]) == -1) { 
			...
			for(int i = 0; i < count; i++) { 
				...
				if(strcmp(args[i], "&") == 0) { has_bg = i; break; }
			}
