...
public:
    ...
    void preorderSearch() { preorderSearch(this->root); }
    void preorderSearch(BinaryTNode<T>* node) {
        if(node == NULL) return;
    ...
    void postorderSearch() { postorderSearch(this->root); }
    void postorderSearch(BinaryTNode<T>* node) {
        if(node == NULL) return;
    ...
    int getSum(int target) { return getSum(target, root->getData(), this->root->getLeft()) + getSum(target, root->getData(), this->root->getRight()); }
    int getSum(int target, int prev, BinaryTNode<T>* node) {
        if(node == NULL) return 0;
        if(node->getData() + prev == target) return 1;
        else return getSum(target, (node->getData() + prev), node->getLeft()) + getSum(target, (node->getData() + prev), node->getRight());
    }
...
int main() {
    ...
    std::cout << tree.getSum(target) << std::endl;
